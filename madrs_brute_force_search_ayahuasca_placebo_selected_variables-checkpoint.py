...
'''
    Scripts for Brute Force search on selected variables from "Ayahuasca vs Placebo" analysis for predicting MADRS D2
...
print(f'Scripts for Brute Force search on selected variables from "Ayahuasca vs Placebo" analysis for predicting MADRS D2')
...
features = 'hematocrit, segmented neutrophils, ldl cholesterol, red blood cells, hemoglobin, leukocytes, triglycerides, alt, bdnf, platelets, ast, lymphocyte, glucose, crp'.split(', ')
...
results.sort_values(by='Ayahuasca', ascending=False).to_csv(f'tables/brute_force_madrs_D2_selected_variables_from_ayahuasca_placebo.csv', index=False)
